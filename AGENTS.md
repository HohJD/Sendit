...
3. The user prefers free models. `openrouter/free` is the default, has zero listed token prices and routes only to free models with the required image/JSON capabilities. On failure OpenRouter falls back to `openai/gpt-4.1-mini` (cheap, paid) unless `LLM_FALLBACK_MODELS` is set to empty; the `model used` log line shows which model answered. Both model calls use this router, but it may select different underlying models. Confirm account access, rate limits and result quality with a controlled live call; mocked tests do not prove these.
